...
- **登入失敗 / 暴力破解成功**:regex 已經照 DSM **記錄查看器**(控制台 →
  ...
  `app/services/synology_log_analyzer.py` 模組開頭的真實範例——**但這條
  規則實機上確認完全不會觸發,是 DSM 沒有轉發、不是程式碼問題,已知
  限制,見下面「已知限制」一節。**
...
### 已知限制:DSM 不會轉發入口網站登入事件

**結論(已確認,不是推測)**:DSM 的 Log Center 不會把「使用者登入 DSM
入口網站」這個事件透過 syslog 轉發出去,即使記錄查看器裡看得到、即使
「記錄傳送設定」的「連線」類別已勾選。「登入失敗次數異常」/「疑似暴力
破解成功登入」這兩條規則因此**目前實質上不會觸發**——這是 DSM 的行為,
不是這個專案的程式碼問題,不用再花時間改 `synology_log_analyzer.py`
的 regex 來解決。

診斷過程(供之後回頭查證):
1. DSM 記錄查看器(控制台 → 記錄中心 → 記錄查看器,篩「連線」類別)
   看得到 `User [G60010] from [192.168.2.156] failed to sign in to
   [DSM] via [password] due to authorization failure.` 這筆記錄,優先
   層級「警告」。
2. 同一時段,`syslog_messages` 裡完全查不到任何含這個帳號、這個 IP、
   或「sign in」字樣的記錄。
3. 但同一時段的檔案操作(`WinFileService Event`)、SMB 存取共用資料夾
   (`Connection: ... accessed shared folder`)都正常送達,連同一個
   帳號(`G60010`)從別的裝置做的檔案操作都收得到——只有入口網站登入
   這個事件類型完全沒有。

如果之後想繼續追這個問題,方向是 DSM 本身的設定/韌體版本行為(例如去
Synology 官方社群或客服確認「入口網站登入事件是否支援 Log Center 轉發」),
不是這個 repo 的程式碼範圍。
